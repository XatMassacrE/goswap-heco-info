...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name uniscam return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name uniscam return';
  }

  @media screen and (max-width: 500px) {
    grid-template-columns: 2.5fr 1fr;
    grid-template-areas: 'name uniscam';
  }
`
        ...
        <DataText area="name" justifyContent="flex-start" alignItems="flex-start">
          ...
          <AutoColumn gap="8px" justify="flex-start" style={{ marginLeft: '20px' }}>
            ...
            <RowFixed gap="8px" justify="flex-start">
              ...
              >
                <ButtonLight style={{ padding: '4px 6px', borderRadius: '4px' }}>添加</ButtonLight>
              </Link>
              {poolOwnership > 0 && (
                <Link external href={getPoolLink(position.pair.token0.id, position.pair.token1.id, true)}>
                  <ButtonLight style={{ padding: '4px 6px', borderRadius: '4px' }}>移除</ButtonLight>
                </Link>
              )}
            </RowFixed>
          </AutoColumn>
        </DataText>
        <DataText area="uniscam">
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.uniscam_RETURN) {
          return p0?.uniscam?.return > p1?.uniscam?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
        }
    ...
    <ListWrapper>
      <DashGrid center={true} style={{ height: '32px', padding: 0 }}>
        ...
        <Flex alignItems="flex-start" justifyContent="flex-start">
          <TYPE.main area="number">名称</TYPE.main>
        </Flex>
        <Flex alignItems="center" justifyContent="flexEnd">
          ...
          >
            {below740 ? '数量' : '流动性市值'} {sortedColumn === SORT_FIELD.VALUE ? (!sortDirection ? '↑' : '↓') : ''}
          </ClickableText>
        </Flex>
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            <ClickableText
              area="return"
              onClick={() => {
                setSortedColumn(SORT_FIELD.uniscam_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.uniscam_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? '手续费收益' : '全部手续费收益'}{' '}
              {sortedColumn === SORT_FIELD.uniscam_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
      <Divider />
      <List p={0}>{!positionsSorted ? <LocalLoader fill="bool" /> : positionsSorted}</List>
      <PageButtons>
